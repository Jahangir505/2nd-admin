...
module.exports = {
    _config: {
      ...
      index: function (req, res) {
        ...
        // =============Embaded Start===============
        return Promise.all([
          BlogCategory.count(qdata),
          BlogCategory.find(qdata).paginate({page: currentPage, limit: pageLimit}).sort('id ' +paSortBy),
        ])
          .spread(function(totalCount, allRecord){
            ...
            return res.view("admin/blog-category/index", {
                ...
                title: 'Category',
                subtitle: 'Index',
                link1: '/admin/blog-category/index/',
                // linkback1: '/admin/BBACKK/index/',
                linknew: '/admin/blog-category/new/',
        
                paginateUrl: '/admin/blog-category/index',
            ...
            return res.redirect('/admin/blog-category/index');
          });
        // =============Embaded End=================
      ...
      new : function (req, res) {
        ...
        return res.view("admin/blog-category/new", {
            flashMsgError: req.flash('flashMsgError'),
            flashMsgSuccess: req.flash('flashMsgSuccess'),
            data: _formData,
            formActionTarget : "/admin/blog-category/create",
            status: 'OK',
            // fieldErrors: {},
            customFieldErrors: {},
            module: 'Blogs',
    ...
    return Promise.all([
        BlogCategory.findOne({name: _formData.name}),
      ...
      //  console.log('main_image_Ret is', main_image_Ret);
        ...
        if (Object.keys(customFieldErrors).length) {
          ...
          return BlogCategory.create(_formData).then(function (createData) {
        ...
        return res.redirect('/admin/blog-category/new');
      });
    // =============Promisses End=======================
      },
      ...
      edit: function (req, res) {
        ...
        BlogCategory.findOne({id:id}).then(function (singledata){
          if (!singledata) {
            req.flash('flashMsgError', 'Data Not Found In Database');
            return res.redirect('/admin/blog-category/index');
          }else{
            return res.view("admin/blog-category/edit", {
              flashMsgError: req.flash('flashMsgError'),
              flashMsgSuccess: req.flash('flashMsgSuccess'),
              data: singledata,
              formActionTarget : "/admin/blog-category/update/" + id,
              status: 'OK',
              fieldErrors: {},
              customFieldErrors: {},
              module: 'Blogs',
     ...
     Promise.all([
        ...
        if (Object.keys(customFieldErrors).length) {
          req.flash('errorMessage', 'Error in Update');
            ...
            //  ==================Start=====================
              return BlogCategory.update({id: id}, _formData).then(function (_updateSingle) {
      ...
      view: function (req, res) {
        ...
        BlogCategory.findOne({id:id}).then(function (singleData){
          if (!singleData) {
            ...
            return res.view("admin/blog-category/view", {
                flashMsgError: req.flash('flashMsgError'),
                flashMsgSuccess: req.flash('flashMsgSuccess'),
                data: singleData,
                moment: moment,
