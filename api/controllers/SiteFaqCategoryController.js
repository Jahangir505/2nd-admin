...
module.exports = {
    _config: {
      ...
      index: function (req, res) {
        ...
        // =============Embaded Start===============
        return Promise.all([
          SiteFaqCategory.count(qdata),
          SiteFaqCategory.find(qdata).paginate({page: currentPage, limit: pageLimit}).sort('id ' +paSortBy),
        ])
          .spread(function(totalCount, allRecord){
            ...
            return res.view("admin/site-faq-category/index", {
                ...
                title: 'Category',
                subtitle: 'Index',
                link1: '/admin/site-faq-category/index/',
                // linkback1: '/admin/BBACKK/index/',
                linknew: '/admin/site-faq-category/new/',
        
                paginateUrl: '/admin/site-faq-category/index',
            ...
            return res.redirect('/admin/site-faq-category/index');
          });
        // =============Embaded End=================
      ...
      new : function (req, res) {
        ...
        return res.view("admin/site-faq-category/new", {
            flashMsgError: req.flash('flashMsgError'),
            flashMsgSuccess: req.flash('flashMsgSuccess'),
            data: _formData,
            formActionTarget : "/admin/site-faq-category/create",
            status: 'OK',
            // fieldErrors: {},
            customFieldErrors: {},
            module: 'Site Events ',
    ...
    return Promise.all([
        SiteFaqCategory.findOne({name: _formData.name}),
      ...
      //  console.log('main_image_Ret is', main_image_Ret);
        ...
        if (Object.keys(customFieldErrors).length) {
          ...
          return SiteFaqCategory.create(_formData).then(function (createData) {
        ...
        return res.redirect('/admin/site-faq-category/new');
      });
    // =============Promisses End=======================
      },
      ...
      edit: function (req, res) {
        ...
        SiteFaqCategory.findOne({id:id}).then(function (singledata){
          if (!singledata) {
            req.flash('flashMsgError', 'Data Not Found In Database');
            return res.redirect('/admin/site-faq-category/index');
          }else{
            return res.view("admin/site-faq-category/edit", {
              flashMsgError: req.flash('flashMsgError'),
              flashMsgSuccess: req.flash('flashMsgSuccess'),
              data: singledata,
              formActionTarget : "/admin/site-faq-category/update/" + id,
              status: 'OK',
              fieldErrors: {},
              customFieldErrors: {},
              module: 'Site FAQs ',
     ...
     Promise.all([
        ...
        if (Object.keys(customFieldErrors).length) {
          req.flash('errorMessage', 'Error in Update');
            ...
            //  ==================Start=====================
              return SiteFaqCategory.update({id: id}, _formData).then(function (_updateSingle) {
     ...
     // ==================Save End=======================
      ...
      view: function (req, res) {
        ...
        SiteFaqCategory.findOne({id:id}).then(function (singleData){
          if (!singleData) {
            ...
            return res.view("admin/site-faq-category/view", {
                flashMsgError: req.flash('flashMsgError'),
                flashMsgSuccess: req.flash('flashMsgSuccess'),
                data: singleData,
                moment: moment,
      ...
      delete : function (req, res) {
        ...
        Promise.all([
            SiteFaqCategory.findOne({id: id}),
            SiteFaq.findOne({siteFaqCategory:id},{select: ['id']}),
        ]).spread(function(deleteData, siteFaqhasData){
          if(!deleteData){
            req.flash('flashMsgError', 'Data Not found In Database');
            return res.redirect('/admin/site-faq-category/index');
          }
